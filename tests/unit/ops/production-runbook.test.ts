...
describe("Milestone 2 production operator runbook", () => {
  ...
  it("requires cold, warm and dual real-CLI capacity evidence and persists it", () => {
    ...
    expect(capacityGate).toContain("capacity-cold-$m2_capacity_stamp.diagnostics.json");
    expect(capacityGate).toContain("capacity-warm-$m2_capacity_stamp.diagnostics.json");
    expect(capacityGate).toContain("capacity-dual-$m2_capacity_stamp.diagnostics.json");
    expect(capacityGate.match(/AGENT_RUNTIME_CAPABILITY_DIAGNOSTICS_OUTPUT/gu)).toHaveLength(3);
    ...
    expect(capacityGate).toContain('value.terminalCode !== "BENCHMARK_COMPLETED"');
    expect(capacityGate).toContain("safeDiagnostics");
    expect(capacityGate).toContain("it is not OOM evidence");
    expect(capacityGate).toContain("safePathFields");
    expect(capacityGate).toContain("diagnostics final status is invalid");
