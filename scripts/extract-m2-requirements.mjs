...
if (!sourcePath) {
  throw new Error("Kullanım: node scripts/extract-m2-requirements.mjs <goal-source.txt>");
}
...
const manifest = {
  source: "Milestone 2 goal supplied by the repository owner",
...
await writeFile(
  path.resolve("docs/m2-requirements.json"),
  `${JSON.stringify(manifest, null, 2)}\n`,
);
await writeFile(path.resolve("docs/M2_REQUIREMENTS.md"), requirementMarkdown);
await writeFile(path.resolve("docs/M2_TRACEABILITY.md"), traceabilityMarkdown);

process.stdout.write(`Generated ${requirements.length} Milestone 2 requirements.\n`);
