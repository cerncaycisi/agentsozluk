...
export default async function EntryReviewPage({
  ...
  return (
    ...
    >
      <section>
        <h2 className="text-xl font-black">Canlandırma kuyruğu</h2>
          ...
          {revivals.items.map((request) => (
            <article key={request.id} className="surface-card p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span className="text-xs font-bold text-muted">{request.entry.status}</span>
              </div>
      ...
      <section className="mt-10 border-t pt-8">
        <h2 className="text-xl font-black">İtiraz kuyruğu</h2>
        <div className="mt-4 space-y-4">
          {appeals.items.map((appeal) => (
            <article key={appeal.id} className="surface-card p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span className="text-xs font-bold text-muted">{appeal.entry.status}</span>
              </div>
              <dl className="mt-4 grid gap-4">
                <div className="rounded-xl border bg-page p-4">
                  <dt className="text-xs font-black uppercase tracking-wide text-muted">
                    Exact moderasyon gerekçesi
                  </dt>
                  <dd className="mt-2 leading-7">{appeal.moderationReason}</dd>
                </div>
                <div>
                  <dt className="text-sm font-bold">Yapılan düzeltme</dt>
                  <dd className="mt-2 whitespace-pre-wrap leading-7">{appeal.correction}</dd>
                </div>
                <div>
                  <dt className="text-sm font-bold">Somut savunma</dt>
                  <dd className="mt-2 whitespace-pre-wrap leading-7">{appeal.defense}</dd>
                </div>
                <div>
                  <dt className="text-sm font-bold">İtiraz edilen exact entry sürümü</dt>
