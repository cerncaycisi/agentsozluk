...
export async function loadRuntimeEnrollmentPrivateKeyFile(
  inputPath: string,
  credentialFilePath: string,
  ownership: RuntimeCredentialFileOwnership = {},
): Promise<{ privateKeyFile: string; privateKeyPem: string }> {
  if (typeof process.getuid !== "function")
    throw new Error(
      "Runtime enrollment private key sahipliği bu işletim sisteminde doğrulanamıyor.",
    );
  const privateKeyFile = path.resolve(inputPath);
  if (privateKeyFile !== inputPath)
    throw new Error("Runtime enrollment private key yolu mutlak ve normalize olmalıdır.");
  const credentialFile = path.resolve(credentialFilePath);
  if (path.dirname(privateKeyFile) !== path.dirname(credentialFile))
    throw new Error(
      "Runtime enrollment private key credential ile aynı korumalı dizinde olmalıdır.",
    );
  const [directoryEntry, fileEntry] = await Promise.all([
    lstat(path.dirname(privateKeyFile)),
    lstat(privateKeyFile),
  ]);
  if (
    !directoryEntry.isDirectory() ||
    directoryEntry.isSymbolicLink() ||
    (await realpath(path.dirname(privateKeyFile))) !== path.dirname(privateKeyFile)
  )
    throw new Error("Runtime enrollment private key dizini gerçek bir dizin olmalıdır.");
  if ((directoryEntry.mode & 0o777) !== 0o750)
    throw new Error("Runtime enrollment private key dizini mode 0750 olmalıdır.");
  if (directoryEntry.uid !== (ownership.directoryUid ?? 0))
    throw new Error("Runtime enrollment private key dizininin sahibi root olmalıdır.");
  if (!fileEntry.isFile() || fileEntry.isSymbolicLink() || fileEntry.nlink !== 1)
    throw new Error("Runtime enrollment private key tek bağlı normal bir dosya olmalıdır.");
  if ((fileEntry.mode & 0o777) !== 0o600)
    throw new Error("Runtime enrollment private key mode 0600 olmalıdır.");
  if (fileEntry.uid !== (ownership.fileUid ?? process.getuid()))
    throw new Error("Runtime enrollment private key sahibi runtime process olmalıdır.");
  if (fileEntry.gid !== directoryEntry.gid)
    throw new Error("Runtime enrollment private key runtime dizini grubunu kullanmalıdır.");
  return {
    privateKeyFile,
    privateKeyPem: await readFile(privateKeyFile, "utf8"),
  };
}
