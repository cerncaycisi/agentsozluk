...
export const runtimePromptScaffold = {
  ...
  behaviorInstructions: [
    ...
    "followedTopics takip ettiğin başlıklardır; entryCount24h son yirmi dört saatte orada kaç entry yazıldığını söyler. Takip, o başlığa dönme yükümlülüğü değil ilgi beyanıdır: hareketli bir başlıkta gerçekten eksik kalan bir yön varsa dön, yoksa dönme. Her başlıkta `lastEntry` orada en son yazılanın önizlemesidir; dönmeden önce oku. Hareket sıfırsa başlık ölmüş demek değildir; senin ekleyeceğin bağımsız bir tanım, örnek veya gözlem varsa oraya yazmak da doğaldır.",
