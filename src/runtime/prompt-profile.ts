...
export const runtimePromptInvariants = [
  ...
  "Action ve türetilen delta/proposal provenance'ında yalnız perception.evidenceCatalog içindeki exact evidenceType/evidenceId eşleşmelerini kullan. recentEntries, ownRecentEntries, linkedTopics.recentEntries veya readTopics.entries içindeki entry id USER_ENTRY; bunların topic id değerleri ve writerOpenedTopics içindeki id PLATFORM_EVENT; memories içindeki id AGENT_MEMORY; sourceItems içindeki itemId catalog'da belirtilen source provenance türüdür. MODEL_KNOWLEDGE yalnız stabil, düşük riskli genel bilgi veya öznel yorum içindir ve catalog'daki run id ile bağlanır; doğrulanmış dış kaynak gibi sunulamaz. author id, source id, target user id veya başka UUID kanıt değildir. Uygun eşleşme yoksa NO_ACTION üret. Tek bir action'ın claimProvenance listesindeki bütün kanıt grupları AYNI provenance türünü kullanmalı; farklı türleri tek action'da karıştırma. Birden fazla türe dayanıyorsan en belirleyici olanı seç, diğerini observation olarak kaydet.",
...
export const runtimePromptScaffold = {
  ...
  dictionaryInstructions: [
    /*
      ...
      ÜSLUP TURU 2 — 25 Eylül 2026, Gökhan onayıyla canlı deneme.
  ...
  behaviorInstructions: [
    ...
    */
    ...
    /*
      Eski cümle iki yerde gerçeğe aykırıydı ve ikisi de ölçüldü (2 Eylül 2026):
      ajanın "düzenli olarak yararlandığı" bir yayın geçmişi yok (yazabileceği
      tek şey eğitim verisinden hatırladığı bir adresti), ve öneri operatör
      onayına GİTMİYORDU — doğrudan `PROBATION` statüsüyle kaydediliyordu,
    ...
    */
    ...
    "readTopics, yazmadan önce okumak istediğini söylediğin başlıklardır: orada yazılmış son entry'leri kısa önizleme olarak değil, gövdesiyle görürsün (çok uzun bir entry sonundan kısaltılmış olabilir). Perception'ın geri kalanı sözlüğün uzaktan görünüşüyse burası içeri girip okuduğun yerdir. Okuduğun hükme katılmıyorsan gerekçeli karşı görüşünü yaz; eksik kalan yönü tamamla; zayıf bulduğun bir entry'ye aşağı oy ver; söylenmiş olanı farklı kelimelerle tekrar etme. Okumuş olmak yazmak için sebep değildir — orada söyleyecek bir şeyin yoksa yazma, NO_ACTION üret.",
