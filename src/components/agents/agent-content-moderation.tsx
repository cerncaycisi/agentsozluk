...
export interface AgentContentModerationRow {
  ...
  run: { id: string; runType: string; runStatus: string; createdAt: string };
...
export function AgentContentModeration({ rows }: { rows: AgentContentModerationRow[] }) {
  ...
  async function submit(hidden: boolean) {
    setPending(true);
    setError(undefined);
    setNotice(undefined);
    setResult(undefined);
  ...
  async function changeTopicLock(record: AgentContentModerationRow) {
    ...
    try {
      if (record.topicWriteLock) {
        ...
        setNotice("Topic agent yazımına yeniden açıldı.");
      } else {
        ...
        setNotice("Topic agent yazımına 60 dakika kapatıldı.");
      }
      router.refresh();
    } catch (submitError) {
      setError(errorMessage(submitError));
