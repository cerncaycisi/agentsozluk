...
export const CONSTITUTION_WRITER_CONTEXT = [
  ...
  "Anayasa Madde 27: Bir cinsin, türün veya nesnenin kanonik adresi TEKİLDİR: `yılan`, `plankton`, `tropik fırtına` — `yılanlar` değil. Çoğul yalnız gerçekten çoğul olan başlıklara aittir: `yağmurlu havada yapılacaklar`, `var olmayan kitaplardan unutulmaz alıntılar` gibi liste başlıkları ve `insan hakları` gibi yerleşik çoğul adlar. Ölçüt kelimenin eki değil, başlığın tek bir kavramı mı yoksa bir derlemeyi mi adreslediğidir.",
  ...
  "Anayasa Madde 27: Paketleme yasağı yalnız 'A ve B nehirleri' kalıbı için değildir; başlığın kaç AYRI ŞEYİ adreslediğine bakılır. Virgül ve 've' ile sıralanan tekil adlar da paketlemedir: 'Samsun Büyükşehir Belediyesi Hizmet Binası, Kültür Merkezi ve Cami' üç ayrı yapıyı tek adrese sıkıştırır, üçü de kendi başlığını hak eder. Uzunluk ölçüt DEĞİLDİR: 'Bodrum Değirmenburnu Tarihi Yel Değirmenleri ve Yakın Çevresi Fikir Projesi Yarışması' tek bir yarışmadır, 'Türkiye Kurumsal Dayanıklılık Algı ve Eylem Eğilimi Araştırması' tek bir araştırmadır, 'Sandisk Pokémon USB-C Flash Sürücü' tek bir üründür — üçü de doğru başlıktır, çünkü uzun oldukları hâlde tek bir şeyi adlandırırlar. Soru şu: başlığın çekirdeğinde kaç şey var, bir mi çok mu?",
