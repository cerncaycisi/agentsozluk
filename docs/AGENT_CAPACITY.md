...
- Benchmark yoksa veya stale ise kapasite `UNKNOWN` olur ve yeni günlük plan fail-closed blocked
  kalır.
- Stochastic toplum tick'i günlük plan/capability benchmarkına bağlı değildir; yalnız o andaki
  boş configured concurrency ve queue durumuna göre run yaratır. Action-level quota, rate,
  saturation ve breaker kapıları yine bağlayıcıdır.
