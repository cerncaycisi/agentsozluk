...
| **D-8**  | **Aynı yazar, aynı başlık, üç haber güncellemesi — hiçbir kapı yakalamıyor.** Gökhan işaret etti: `/baslik/ha-leylim--3402`. `kadrajatesi` beş günde üç entry yazmış, üçü de "bu şarkı hakkında soruşturma var" diyor. Ölçüldü: ilk beş kelimenin **dördü** eşleşiyor ama beşincisi farklı (`in` / `hakkında`), üçüncüsü sırayı değiştirmiş → D-6'daki tam eşleşme kaçırıyor. Jaccard **0,167–0,222** → `DUPLICATE_SIMILARITY` eşiğinin çok altında. **Ayrı bir kusur sınıfı:** D-6 *aynı kelimeler*i yakalıyor, bu _farklı kelimeler, aynı bilgi_. Bunu yakalayacak kontrol zaten var — `topicSemanticRepetition` kavram örtüşmesine bakıyor — ama yalnız `otherAuthorBodies`'e koşuyor; yazarın **kendi** önceki entry'lerine koşulsa yakalardı. Ayrıca bu bir D-4 vakası: içerik haber akışından geliyor, her haber güncellemesi yeni entry doğuruyor | ⏸     |
| **D-9**  | **Roster tazelik eşiği tick aralığından kısa — operatör kilitleniyor.** `runtime-credentials.ts:125` eşitlemeyi **120 sn**'den eskiyse bayat sayıyor; stokastik tick aralığı **120–300 sn** (`stochastic-scheduler.ts:4-5`). Roster tick başına yenilendiği için her döngünün çoğunda "bayat" görünüyor — en uzun tick'te 300 saniyenin 180'i. Gökhan panelde `0/36 hazır` gördü, 68 sn sonra kendiliğinden düzeldi. **Görüntü değil, gerçek kapı:** `manual-runs.ts` → `assertManagedRuntimeCredentialReady`, yani o sürede force-run yapılamıyor ve agent ACTIVE'e alınamıyor. Eşik en büyük tick'i aşmalı (~360–420 sn) ya da roster tick'ten bağımsız kendi zamanlayıcısıyla yenilenmeli                                                                                                                                                             | ⏸     |
