...
| P3  | Koyu temayı ayarla                                                     | 🔄 ui/koyu-tema | `page`/`surface` 1.075:1; "Kayıt ol" fazla baskın; kenarlıklar ağır                                                                                                                                                                                                                                                                                                                    |
...
| S4  | **Üçüncü yazma yüzeyi**: entry düzenleme formu                         | 🔄 ui/cila      | `entry-actions.tsx:567` — araç çubuğu var, önizleme yok, `ENTRY_BODY_MAX_LENGTH`'i üçüncü kez kopyalıyor. `EntryComposerField`'ın doğal üçüncü müşterisi                                                                                                                                                                                                                               |
...
| S6  | `baslik/[topic]/page.tsx` kardeş durumları hâlâ `surface-card`         | 🔄 ui/cila      | Giriş CTA'sı, onay bekliyor, askıya alınmış. Composer'la aynı anda görünmüyorlar, çelişki yok ama hizalanmalı                                                                                                                                                                                                                                                                          |
| S7  | Yerel dev DB'sinde test verisi kaldı                                   | 🔄 ui/cila      | `paylasilan composer denemesi *` başlıkları. Görsel incelemeden önce temizle                                                                                                                                                                                                                                                                                                           |
| S2  | `.chip` durgunken 3:1 kenarlık taşımıyor                               | 🔄 ui/koyu-tema | P0.1'den çıktı: `.chip` yalnız hover'da `--border-strong`'a ulaşıyor. Sistem geneli bir karar, yeni pencere tetikleyicisi ve ikon butonları da etkiliyor                                                                                                                                                                                                                               |
| S3  | Zaman etiketi iki kez görünüyor                                        | 🔄 ui/cila      | Eyebrow "7 entry · son 24 saat" derken tetikleyici de "son 24 saat" diyor. Görsel incelemede karar vereceğim                                                                                                                                                                                                                                                                           |
| S10 | **`text-link` ölü bir sınıf**                                          | 🔄 ui/koyu-tema | Ne `tailwind.config.ts`'te ne `globals.css`'te tanımı var. 4 kullanım: `app/kurallar/page.tsx:45`, `app/hakkinda/page.tsx:44`, `content/constitution-document.tsx:34,85`. **Bu bağlantılar şu an gövde metni renginde** — yani bağlantı olduğu belli değil (WCAG 1.4.1). Doğru karşılık `.link-strong` ama /kurallar'daki ~60 madde bağlantısını birden kiremite çevirmek görsel karar |
| S11 | Örtüyü geometriden ayıran bir sınıf gerek                              | 🔄 ui/koyu-tema | `.menu-item` tipografi taşıyor (`text-sm px-3 py-2 rounded-lg w-full`); `topic-list.tsx` ve `theme-settings.tsx` 17px gövde metniyle akan satırlar, uymuyor. Depoda kalan tek `hover:bg-page` bu ikisi — koyu temada görünmüyor                                                                                                                                                        |
...
| S14 | Tema düğmesi koyu temada daha şiddetli hover alıyor                    | 🔄 ui/koyu-tema | `aria-pressed="true"` olduğu için `.icon-button` onu "dolgulu seçili" sayıp 0.08 yerine 0.12 örtü uyguluyor. Kontrast sorunu yok, tutarsızlık var                                                                                                                                                                                                                                      |
| S15 | `site-shell.tsx:51` `footerLinkClass` sisteme geçmedi                  | 🔄 ui/cila      | Geçirilince `site-shell.test.tsx:542` literal sınıf araması düşüyor ve hover `--primary`'den `--ink`'e iniyor. Test + görsel karar gerekiyor                                                                                                                                                                                                                                           |
