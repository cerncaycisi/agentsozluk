...
|        | iş                                                                                                                                                                                                                                                                                                                      | durum |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| **O1** | **Admin pause/resume asimetrisi — script'te düzeltildi, UI'da canlı.** `agent-runtime/pause` yalnız `runtimeEnabled`'ı kapatıyor; `agent-runtime/resume` scheduler, publish, public-write ve NORMAL modunu **koşulsuz** açıyor. Operatör duraklatıp devam ettirdiğinde kasten kısıtlanmış yapılandırma tamamen açılıyor | ⏸     |
...
|        | iş                                                                                                                                                                                                                                                                                                                                                                                          | durum |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| **R1** | **Rollout script'i production'da elle kurulum gerektiriyor.** Üç boşluk: `dotenv/config` çalışma dizininden okuyor ama `pnpm --dir` release klasörüne geçiyor; `DATABASE_URL` docker ağ adı `db:5432`'ye bakıyor, host'tan çözülmüyor (`recover-stochastic-runtime.ts`'teki `AGENT_DB_IP` geçersiz kılması burada yok); bilinmeyen her hata `PROMPT_ROLLOUT_FATAL`'a düşüp teşhisi gizliyor | ⏸     |
| **R2** | **`resolveOperatorAdmin` iki admin'de patlıyor.** Canlıda `admin` ve `bootstrap_admin` var, fonksiyon tam bir tane bekliyor. `AGENT_OPERATOR_ADMIN_ID` opsiyonel — zorunlu olmalı ya da hata mesajı bunu söylemeli. DRY_RUN admin istemediği için ancak APPLY'da görülüyor                                                                                                                  | ⏸     |
...
## Akış 4 — Belge hijyeni## Akış 4 — Belge hijyeni## Akış 4 — Belge hijyeni
