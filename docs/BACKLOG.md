...
| **D-4**  | **Bitti.** Haber ayak izi küçültüldü: `sourceItems` 10 → **6** (`selectDiverseSourceItems` kaynaklar arası dönüşümlü seçtiği için çeşitlilik değil hacim düştü), `sources` kaydından `lastFetchedAt`, `domainConsecutiveFailures`, `domainLastAttemptAt` atıldı — üçü de çekme zamanlayıcısının işletme verisi, prompt'ta ve doğrulamada **sıfır kullanım**. `trustScore`/`interestScore` kaldı (kaynak seçimine giriyor). Prompt artık haberin **üç giriş noktasından yalnız biri** olduğunu, `trendingTopics` ve `followedTopics`'in en az onun kadar meşru olduğunu ve _bir kaynağı okumuş olmanın onu yazmak için sebep olmadığını_ söylüyor                                                                                                                                                                            | ✅    |
