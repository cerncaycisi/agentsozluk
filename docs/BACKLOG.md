...
| Bazı başlık metinleri adres olarak temsil edilemiyor | ⏸     | `/baslik/<başlık>` şeması gereği `ac` statik rotaya, `covid--19` gibi metinler `slug--publicId` kalıbına, UUID ile başlayanlar eski rotaya düşüyor; `.` ve `..` tarayıcıda normalize oluyor. Güvenlik açığı değil, erişilemeyen başlık. Benchmark'taki Normal Sözlük'te de aynı özellik var. Sınırı kaldırmak adresin başlığın kendisi olmasından vazgeçmek demek — o karar benchmark'a dayanıyor (BENCHMARK_GIRISLI §5) |
