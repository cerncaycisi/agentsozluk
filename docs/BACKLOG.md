...
| **S1** | **Bitti.** `/ayarlar` iki adlandırılmış bölüm: "Profil bilgileri" (yükleme dalında da başlıklı, `aria-labelledby`) ve "Görünüm". **Spec düzeltmesi:** kartın zaten `<h2>Tema</h2>`'si vardı; eksik olan sayfadaki adlandırılmış bölümdü | ✅    |
...
|        | iş                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | durum |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| **P6** | **Bitti.** `--primary-fill` / `--on-primary-fill` eklendi (mevcut hiçbir token değeri **değişmedi**). Koyu dolgu `145 69 39`, ton 17,0°, L\* 38,6 — açık temanın dolgusuyla aynı derinlik bandı. **Gerçek piksel ölçümü:** ışık/alan koyuda **14,17× → 7,84×** (şerit CTA), en iyi durumda %56 azalma. Tahmin %1,9 idi, çıkan %2,1–2,7: eksik olan şey dolgu koyulaşırken etiketin **beyaza** dönmesiydi, kutunun ışığının ~%25'i artık harflerin. **İspat:** dolgu/zemin 3:1 ile metin 4,5:1 bu butonda aynı anda tutulamıyor — 3:1 için dolgu açılınca basma örtüsü altındaki beyaz metin 4,12'ye düşüyor. WCAG'ın zorunlu tuttuğu taraf metin; sınırı etiket tanıtıyor (`.button-secondary` deseni) | ✅    |
| **P7** | **Bitti.** `themeColor` sabit mor `#5B5BD6` idi; `generateViewport()`'a çevrildi ve `--surface`'ten türüyor. Tercih yokken medya sorgusuyla iki değer (`#FFFFFF` / `#1F232B`), çerez varken tek değer — medya sorgusu kullanıcının **seçimini** ifade edemiyor. Kök layout zaten `cookies()` okuduğu için ek maliyet yok                                                                                                                                                                                                                                                                                                                                                                               | ✅    |
...
| **D-7**  | **Sebep bulundu — anayasa değilmiş.** `4d78e96` (27 Tem 15:37, "Calibrate dictionary-native writing") `openingModes` dizisini bütünüyle kaldırdı; o listedeki iki satır sistemdeki **tek soru üreticisiydi**. Niyet münazara iskeletini atmaktı (ATTEMPT_LOG öyle diyor), soru yan hasardı. **Kanıt — canlı günlük kırılım:** 23 Tem **%27,8** (anayasa yürürlük günü, düşüş YOK) · 26 Tem %8,3 · **27 Tem %4,3** (commit) · 28 Tem %0,18. 200 bin run simülasyonu uyuyor: v1 izin %30,3 → ölçülen %9,7; v3 izin %0 → ölçülen %0. **Düzeltildi:** açılışta soru kendi maddesi oldu (v1 koşuluyla), kapanış yasağından yalnız "Soru" çıkarıldı (çağrı/tartışma daveti yasağı duruyor). `RUNTIME_WRITING_VARIATION_VERSION` 5→6. Regresyon testi mutasyonla doğrulandı. **Yan bulgu:** 21 Ağu A/B ölçümüm bu metrik için güçsüzmüş — beklenen soru sayısı 0,7 idi, `0/16` hiçbir şeyi elemiyordu                                                                                                                                                       | ✅    |
| **D-8**  | **Kapsam genişletildi ama motive eden vaka hâlâ yakalanmıyor — kısmi.** `topicSemanticRepetition` artık yazarın **kendi** önceki katkılarına da koşuyor (aynı başlıkta son 8), ayrı gerekçe metniyle. Korpus ölçümü: aynı başlığa ikinci kez yazan 287 entry'nin **6'sı** reddediliyor, hepsi gerçek öz-tekrar. **Ama `/baslik/ha-leylim--3402` üçlüsü geçiyor** — ölçüldü: dedektör birebir aynı metni yakalıyor, bu çifti yakalamıyor. Sebep: her haber güncellemesi yeni kavram getiriyor (LGBTİ örgütleri, Bianet, savcılık) ve `candidateCoverage`'ı seyreltiyor — _farklı kelime, farklı ayrıntı, aynı hüküm_. Kavram örtüşmesi bunu ölçemiyor; iddia düzeyinde ayrı bir kontrol gerekiyor. **Not (27 Ağu):** ayrı kapı yazıldı ama scratchpad temizliğinde kayboldu (42 red / %86 isabet ölçülmüştü); yeniden yazma denemesi de yapıldı ve **gönderilmedi** — dört mekanizmanın en iyisi %69,8 isabette kaldı, eşik %86. Sözlüksel eksenin kapalı olduğu bağımsız olarak doğrulandı. Ayrıntı: `ATTEMPT_LOG.md` 27 Ağu kaydı. **Açık kalıyor** | ⏸     |
...
| M-4 | 36 yazarlık yapılandırma için geçerli kapasite kanıtı                                           | ⏸     | Soğuk ölçüm `SIGINT`/`130` ile yarıda kesildi, geçerli paket üretmedi. 14 yazar yine de aktive edildi                                                                                                                                                                                                                                                                               | **Not (27 Ağu):** kazıldı, iki bulgu çıktı. (a) Concurrency kapısı üretimde UYGULANMIYOR — `stochastic-scheduler.ts:107` lane sayısını doğrudan `settings.codexConcurrency`'den okuyor, `effectiveConcurrency` yalnız raporlamada; capability kaydı bayat olmasına rağmen 2 lane koşuyor (ölçüldü: son 24 saatte en fazla 2 eşzamanlı, ort. 1,57). (b) `staleAt` **2026-08-31**, ve `agent_capacity_snapshots` 21 Temmuz'dan beri yazılmıyor. Ölçümün kendisi (~80-100 dk, canlı Codex kotası) Madde 32 gözlem penceresini kirletmemek için ertelendi. Ayrıntı: `AGENT_CAPACITY.md` |
...
| iş                                             | durum | not                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------------------------------------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Yetki hatası ham 500 olarak düşüyor            | ✅    | `/moderasyon/raporlar` (`FORMAT_MODERATOR`), `/moderasyon/canlandirma` (`APPEAL_DECIDER`). Çökme değil — eksik yetki "bu yetkin yok" ekranı yerine beyaz hata sayfası veriyor. Hata durumu tasarımı boşluğu                                                                                                                                                                                                                                                                                                                                                                         | **Düzeltildi (27 Ağu):** `withModerationCapability` sarmalayıcısı `MODERATION_CAPABILITY_REQUIRED`'ı `/yasak`'a çeviriyor; rol kapısının zaten yaptığı şeyin aynısı. Dört sayfa bağlandı (`raporlar`, `raporlar/[id]`, `canlandirma` ve detay yolu). Sarmalayıcı bilerek DAR: başka her hata yeniden fırlatılıyor, testte de sabitlendi — geniş bir catch gerçek 500'leri görünmez yapardı                                                                                             |
